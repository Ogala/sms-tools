...
def computeEngEnv(inputFile, window, M, N, H):
    ...
    lowBin = int(np.ceil(float(3000)*N/fs))
    highBin = int(np.ceil(float(10000)*N/fs))
    lowBand = mX[:, 1:lowBin]
    highBand = mX[:, lowBin:highBin]

    linear1 = np.power(10, lowBand/20.0)  # Convert to linear
    linear2 = np.power(10, highBand/20.0)  # convert to linear
    enLow = 10*np.log10(np.sum(np.square(linear1), axis=1))

    enHigh = 10*np.log10(np.sum(np.square(linear2), axis=1))

    engEnv = np.zeros((frames, 2))
    engEnv[:, 0] = enLow
    engEnv[:, 1] = enHigh
    return engEnv
...
(env) = computeEngEnv(inputFile, window, M, N, H)

...
def extractMainLobe(window, M):
    ...
    ### Your code here
    hM1 = int(math.floor(M/2))
    hM2 = int(math.floor(M/2))
    N = int(8*M) # Signal size
    ...
    x = fft(fftBuffer)
    absX = abs(x)
    absX[absX < np.finfo(float).eps] = np.finfo(float).eps #replace all zero values with eps
    mx = 20*np.log10(absX) # Convert spectrum to decibels

    # Undo the zero phase windowing so we can display the array properly
    mX1 =  np.append(mx[-hN:], mx[:hN])

    # Search for first lowest point from max
    minIndexDist = 0
    for i in range(hN):
        n = hN+i
        if mX1[n+1] > mX1[n]:
            minIndexDist = i
            break
    mF = mX1[int(hN-minIndexDist):int(hN+minIndexDist+1)]
    return mF

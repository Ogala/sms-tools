...
def segmentStableNotesRegions(inputFile = '../../sounds/sax-phrase-short.wav', stdThsld=10, minNoteDur=0.1, 
    ...
    wOffset  = winStable - 1
    stds = np.zeros(wOffset)
    for i in range(wOffset, f0Cents.size):
        stds = np.append(stds, np.std(f0Cents[i - wOffset: i+1]))
    ...
    if len(stables) > 0:
        ...
        for stable in stables:
            if stable != (sequence[0] + index):
                segments.append([sequence[0], sequence[len(sequence) -1]])
                sequence = []
            sequence.append(stable)
            index = len(sequence)

    #5. apply segment filtering, i.e. remove segments with are < minNoteDur in length
    filteredSegments = []
    for i in segments:
        iNoteDur =  (i[1] - i[0])*H/float(fs)
        if iNoteDur > minNoteDur:
            filteredSegments.append(i)
